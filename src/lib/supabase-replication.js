...
class SupabaseReplication extends RxReplicationState {
  constructor(options) {
    ...
    if (
      options.pull &&
      (options.pull.realtimePostgresChanges ||
        typeof options.pull.realtimePostgresChanges === "undefined")
    ) {
  ...
  setupRealtimeChannel() {
    this.realtimeChannel =
      this.options.supabaseClient.channel(
        `rxdb-supabase-${this.replicationIdentifierHash}` 
      );
  ...
  async pullHandler(lastCheckpoint, batchSize) {
    ...
    if (this.table === "transactions") {
      ...
      if (saleItemsResult.data) {
        saleItemsResult.data.forEach((item) => {
          if (!saleItemsMap[item.transaction_id]) {
            saleItemsMap[item.transaction_id] = [];
          }
      ...
      if (expensesResult.data) {
        expensesResult.data.forEach((expense) => {
          if (!expensesMap[expense.transaction_id]) {
            expensesMap[expense.transaction_id] = [];
          }

          expensesMap[expense.transaction_id].push(expense);
        });
      }
      ...
      enrichedData = data.map((txn) => ({
        ...
        expense: expensesMap[txn.id]?.[0] ?? null,
      }));
    }
    ...
    return {
      checkpoint: this.rowToCheckpoint(
        mappedData[mappedData.length - 1]
      ),
      documents: mappedData.map((row) => this.rowToRxDoc(row)),
    };
  }

  async pushHandler(rows) {
    console.log("PUSH RUNNING", this.table, rows);
    ...
    if (this.table === "transactions") {
      if (doc.type === "stock_take_create" && doc.stock_take) {
        const { error: stockTakeError } =
          await this.options.supabaseClient
            .from("stock_takes")
            .insert(doc.stock_take);

        if (
          stockTakeError &&
          stockTakeError.code !== POSTGRES_DUPLICATE_KEY_ERROR_CODE
        ) {
      ...
      if (
        doc.type === "stock_take_submit_counts" &&
        doc.stock_take_id &&
        Array.isArray(doc.stock_take_items)
      ) {
        if (doc.stock_take_items.length > 0) {
          const { error: stockTakeItemsError } =
            await this.options.supabaseClient
        ...
        const { error: stockTakeStatusError } =
          await this.options.supabaseClient
            .from("stock_takes")
      ...
      if (doc.type === "stock_take_approve" && doc.stock_take_approval?.stockTakeId) {
        const { error: stockTakeApproveError } =
          await this.options.supabaseClient
            .from("stock_takes")
            .update({
              status: "approved",
      ...
      if (doc.type === "sale" && Array.isArray(doc.sale_items) && doc.sale_items.length > 0) {
        ...
        const { error: expenseError } =
          await this.options.supabaseClient
            .from("expenses")
            .insert(stripExpenseFields(doc.expense, txn.id));
        ...
        const { error: transferError } =
          await this.options.supabaseClient
            .from("transfers")
            .insert(stripTransferFields(doc.transfer));

        if (transferError) throw transferError;

        if (doc.costExpense?.transaction && doc.costExpense?.expense) {
          const costTxn =
            stripTransactionFields(doc.costExpense.transaction);

          const { data: costTxnRow, error: costTxnError } =
            await this.options.supabaseClient
              .from(this.table)
              .insert(costTxn)
              .select()
          ...
          const { error: costExpenseError } =
            await this.options.supabaseClient
              .from("expenses")
              .insert(
                stripExpenseFields(
                  doc.costExpense.expense,
                  costTxnRow.id
                )
              );
  ...
  async handleUpdate(row) {
    ...
    return [
      await this.fetchByPrimaryKey(
        row.newDocumentState[this.primaryKey]
      ),
    ];
  }
